...
class YesterdayClose:
    ...
    def get_matches(self, data: dict) -> None:
        ...
        close_diff_matchs = self.make_closediff_query()
        if len(close_diff_matchs) > 0:
            print(str(len(close_diff_matchs)) + " match(es) on closediff")
            matches.extend(close_diff_matchs)
        pct_close_diff_matchs = self.make_pctclosediff_query()
        if len(pct_close_diff_matchs) > 0:
            print(str(len(pct_close_diff_matchs)) + " match(es) on pctclosediff")
            matches.extend(pct_close_diff_matchs)
        print("---------------------------------")
